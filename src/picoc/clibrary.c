...
/* probably not needed. just checking */
#include <stdarg.h>
#include <stddef.h>
...
void LibRandomUpper(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = floor(((rand()%RAND_MAX)/(double)RAND_MAX)*Param[0]->Val->Integer)+1;
}

void LibRandom(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = floor(((rand()%RAND_MAX)/(double)RAND_MAX)*(Param[0]->Val->Integer-Param[1]->Val->Integer+1))+Param[1]->Val->Integer;
}

void LibRandomSeed(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
...
struct LibraryFunction CLibrary[] =
{
    ...
    { LibRound,         "double round(double);" },
    { LibCeil,          "float ceil(float);" },
    { LibFloor,         "double floor(double);" },
#endif
    { LibRandomUpper,   "int random_upper(int);" },
    { LibRandom,        "int random(int, int);" },
    { LibRandomSeed,    "void randomseed(int);" },
